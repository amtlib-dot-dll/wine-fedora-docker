...
RUN useradd -U -m user; \
    ...
    dnf install -y winehq-devel $(dnf repoquery --requires winetricks | grep -v ^wine) mesa-dri-drivers.x86_64 mesa-dri-drivers.i686 /usr/bin/ntlm_auth glibc-langpack-en langpacks-en glibc-langpack-zh langpacks-zh_CN langpacks-zh_TW; \
    dnf clean all; \
